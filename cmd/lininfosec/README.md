...
```bash
curl -X POST http://lininfosec.domain/monitor/add -d '{
	"configurationUid": "host1.foo.bar",
	"cpes": [
		"cpe:2.3:a:foocorp:foo:1.3.4:*:*:*:*:*:*:*",
		"cpe:2.3:a:barcorp:bar:1.12:*:*:*:*:*:*:*"
...
```bash
curl -X GET http://lininfosec.domain/monitor/get?name=host1.foo.bar
```
...
```bash
curl -X POST http://lininfosec.domain/monitor/remove -d '{
	"configurationUid": "host1.foo.bar"
}'
```
...
```bash
curl -X POST http://lininfosec.domain/monitor/update -d '{
	"configurationUid": "host1.foo.bar",
	"cpes": [
		"cpe:2.3:a:foocorp:foo:1.3.4:*:*:*:*:*:*:*",
		"cpe:2.3:a:barcorp:bar:1.12:*:*:*:*:*:*:*"
...
```bash
curl -X GET http://lininfosec.domain/searchCPE?query=firefox 
```
